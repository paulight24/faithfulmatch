"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.LegalController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const public_decorator_1 = require("../../common/decorators/public.decorator");
const legal_content_1 = require("./legal.content");
let LegalController = class LegalController {
    privacy(res) {
        res.type('html').send(legal_content_1.PRIVACY_HTML);
    }
    terms(res) {
        res.type('html').send(legal_content_1.TERMS_HTML);
    }
};
exports.LegalController = LegalController;
__decorate([
    (0, public_decorator_1.Public)(),
    (0, common_1.Get)('privacy'),
    __param(0, (0, common_1.Res)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], LegalController.prototype, "privacy", null);
__decorate([
    (0, public_decorator_1.Public)(),
    (0, common_1.Get)('terms'),
    __param(0, (0, common_1.Res)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], LegalController.prototype, "terms", null);
exports.LegalController = LegalController = __decorate([
    (0, swagger_1.ApiExcludeController)(),
    (0, common_1.Controller)('legal')
], LegalController);
//# sourceMappingURL=legal.controller.js.map